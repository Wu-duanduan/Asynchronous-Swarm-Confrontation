...
class IIFDS:
    ...
    def get_alive_red_agents(self, flag_car):
        """获取红方存活的无人机编号"""
        return [i for i in range(int(self.numberofcar / 2), self.numberofcar) if flag_car[i] == 0]

    def get_nearest_cars(self, car_id, alive_ally, opp, positions):
        """
        ...
        # 计算与其他友方车的距离
        ally_distances = []
        for other_id in alive_ally:
            if other_id != car_id:
                dist = np.linalg.norm(positions[car_id] - positions[other_id])
                ally_distances.append((other_id, dist))

        # 计算与敌方车的距离
        enemy_distances = []
        for other_id in opp:
            dist = np.linalg.norm(positions[car_id] - positions[other_id])
            enemy_distances.append((other_id, dist))

        # 按距离排序并选择最近的两辆车
        ally_distances.sort(key=lambda x: x[1])
        enemy_distances.sort(key=lambda x: x[1])

        nearest_ally = [id for id, _ in ally_distances[:2]]
        nearest_enemy = [id for id, _ in enemy_distances[:2]]
        # 如果蓝方或红方存活车辆不足两辆，用-1填充
        nearest_ally = (nearest_ally + [-1, -1])[:2]
        nearest_enemy = (nearest_enemy + [-1, -1])[:2]

        return nearest_ally, nearest_enemy

    def get_obs(self, car_id, alive_ally, all_opp, q, v, flag_car, hp):
        obs = []
        obs_mask = []  # 添加掩码
        uncertainty = []  # 添加不确定性信息

        if flag_car[car_id] == 0:
            nearest_ally, nearest_enemy = self.get_nearest_cars(car_id, alive_ally, all_opp[car_id], q)

            # 处理友军信息
            for j in nearest_ally:
                if j != -1:
