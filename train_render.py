...
from dqn_rl import MultiAgentDQN
...
def get_args():
    ...
    # 训练参数
    parser.add_argument("--max-episode", type=int, default=1, help="最大训练回合数")
    ...
    for episode_gone in range(max_episode):
        ...
        for step in range(max_step):
            ...
            for j in range(int(iifds.numberofcar / 2)):
                # 获取每个智能体的观察
                obs[j], _, _ = iifds.get_obs(j, alive_blue, all_opp, q, v, flag_car, HP_index)
                obs_dim = dqn_params['obs_dim']
                ...
                if step == 0:
                    ...
                    if flag_car[j] == 0:
                        ...
                        # if task_index[j] == 0 and (all_opp[j] == []):
                            flag_decision.append(1)
                            decision_count[j] += 1
                        elif task_index[j] == -2 and (all_opp[j] == [] or situation_enemy >= 0):
                        # elif task_index[j] == -2 and (all_opp[j] == []):
                            flag_decision.append(1)
                            decision_count[j] += 1
                        ...
                        # elif task_index[j] == -1 and (all_opp[j] != [] or all_nei_c2e == []):
                            flag_decision.append(1)
                            decision_count[j] += 1
                        else:
            ...
            # 获取动作并更新任务目标
            actions_refresh = trainer.get_actions(obs, flag_decision, eval_mode=False)  # 使用评估模式
            ...
            env.render(q, v, fire_car, flag_car, HP_index, iifds.HP_num, missle_index,
                       iifds.missle_num, task_index)  # 画出当前一时刻的无人车的位置速度、血量、弹药
            ...
            if (step+1) % 30 == 0:
                ...
                filename = f"./Render_fig/frame-{step}-@sec.png"

                cv2.imwrite(filename, cv2.cvtColor(img_data, cv2.COLOR_RGB2BGR))  # RGB 转 BGR 保存
            ...
            rewards, rewards_loss = getReward1(iifds, actions, obs, global_reward, all_opp, all_nei_c2e, dqn_params['obs_dim'],
                                 rewards)
            ...
            if not (episode_gone == 0 and step == 0):
                ...
                for car_id in range(blue_agents):
                    if flag_decision[car_id] == 1 and flag_car[car_id] == 0:
                        experiences.append((obs_origin[car_id], action_origin[car_id], rewards[car_id], next_obs[car_id],
                                            blue_dones[car_id]))
                        obs_origin[car_id] = obs[car_id]
