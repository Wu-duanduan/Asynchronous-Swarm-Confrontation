...
from transformer_qmix_tf import MultiAgentTransformer
...
def get_args():
    ...
    # Transformer参数
    parser.add_argument("--d-model", type=int, default=64, help="Transformer模型维度")
    parser.add_argument("--nhead", type=int, default=4, help="Transformer注意力头数")
    parser.add_argument("--num-layers", type=int, default=2, help="Transformer层数")
    parser.add_argument("--sequence-length", type=int, default=10, help="Transformer序列长度")
    parser.add_argument("--dropout", type=float, default=0.1, help="Transformer Dropout率")

    # 训练参数
    parser.add_argument("--max-episode", type=int, default=100, help="最大训练回合数")
    parser.add_argument("--max-step", type=int, default=300, help="每回合最大步数")
...
if __name__ == "__main__":
    ...
    # 初始化Transformer强化学习参数
    transformer_params = {
        'obs_dim': 12,  # 观察维度
        'action_dim': 4,  # 动作维度
        'd_model': args.d_model,  # Transformer模型维度
        'nhead': args.nhead,  # 注意力头数
        ...
        'warmup_episodes': 30,  # 预热回合数
        'sequence_length': args.sequence_length  # 序列长度
    }

    # 初始化多智能体DQN强化学习
    trainer = MultiAgentTransformer(blue_agents, transformer_params, device)
    ...
    for episode_gone in range(max_episode):
        ...
        # 初始化状态变量
        episode_rewards = np.zeros(blue_agents)  # 每个智能体累计奖励
        total_reward = 0  # 总奖励
        ...
        # 初始化观察
        obs_origin = [None] * int(iifds.numberofcar / 2)
        action_origin = [None] * int(iifds.numberofcar / 2)
        # 记录每回合的动作分布
        action_counts = {0: 0, 1: 0, 2: 0, 3: 0}  # 0:搜索, 1:逃跑, 2:追击, 3:支援
        step_losses = []
        loss = 0
        rewards = np.zeros(int(iifds.numberofcar / 2))
        decision_count = np.zeros(int(iifds.numberofcar / 2))
        for step in range(max_step):
            ...
            for j in range(int(iifds.numberofcar / 2)):
                # 获取每个智能体的观察
                single_obs, single_mask, single_uncertainty = iifds.get_obs(j, alive_blue, all_opp, q, v, flag_car,
                                                                            HP_index)
                # Ensure single_obs is a 1D array of length obs_dim
                if isinstance(single_obs, list):
                    single_obs = np.array(single_obs, dtype=np.float32)
                if single_obs.ndim != 1 or single_obs.shape[0] != transformer_params['obs_dim']:
                    raise ValueError(
                        f"single_obs for agent {j} has shape {single_obs.shape}, expected ({transformer_params['obs_dim']},)")
                obs.append(single_obs)
                # Ensure single_mask is consistent
                if single_mask is not None:
                    if isinstance(single_mask, list):
                        single_mask = np.array(single_mask, dtype=np.float32)
                    if single_mask.ndim != 1 or single_mask.shape[0] != transformer_params['obs_dim']:
                        raise ValueError(
                            f"single_mask for agent {j} has shape {single_mask.shape}, expected ({transformer_params['obs_dim']},)")
                obs_mask.append(single_mask)
                uncertainty.append(single_uncertainty)
                obs_dim = transformer_params['obs_dim']
                situation_enemy = obs[j][int(2 * obs_dim / 4 + 1)] * 3 + obs[j][int(3 * obs_dim / 4 + 1)]  # 敌方态势得分（有利态势追击，反之逃跑）
                situation_nei = obs[j][int(0 * obs_dim / 4 + 1)] * 3 + obs[j][int(1 * obs_dim / 4 + 1)]  # 友方态势得分（相对朝向友方时支援，反之搜索）
                if step == 0:
                    flag_decision.append(1)
                    decision_count[j] += 1
                else:
                    if flag_car[j] == 0:
                        if task_index[j] == 0 and (all_opp[j] == [] or situation_enemy < 0):
                            flag_decision.append(1)
                            decision_count[j] += 1
                        elif task_index[j] == -2 and (all_opp[j] == [] or situation_enemy >= 0):
                            flag_decision.append(1)
                            decision_count[j] += 1
                        elif task_index[j] == -3 and (all_opp[j] != [] or all_nei_c2e != []):
                            flag_decision.append(1)
                            decision_count[j] += 1
                        elif task_index[j] == -1 and (all_opp[j] != [] or all_nei_c2e == [] or situation_nei < 0):
                            flag_decision.append(1)
                            decision_count[j] += 1
                        else:
                            flag_decision.append(0)
                    else:
                        flag_decision.append(0)

            # Convert to global observations
            global_obs = np.array(obs, dtype=np.float32)  # [num_agents, obs_dim]
            if global_obs.shape != (blue_agents, transformer_params['obs_dim']):
                raise ValueError(
                    f"global_obs shape {global_obs.shape} does not match expected ({blue_agents}, {transformer_params['obs_dim']})")
            global_obs_mask = np.array(obs_mask, dtype=np.float32) if obs_mask[0] is not None else None
            if global_obs_mask is not None and global_obs_mask.shape != global_obs.shape:
                raise ValueError(
                    f"global_obs_mask shape {global_obs_mask.shape} does not match global_obs shape {global_obs.shape}")

            print('decision_count:', decision_count)
            # 获取动作并更新任务目标
            actions_refresh = trainer.get_actions(global_obs, flag_decision, eval_mode=False, obs_masks=global_obs_mask)  # 使用评估模式
            print('actions_refresh:', actions_refresh)
            idx = 0
            actions = []
            for j in range(int(iifds.numberofcar / 2)):
                if flag_decision[j] == 1:
                    actions.append(actions_refresh[idx])
                    idx += 1
                    rewards[j] = 0
                else:
                    if task_index[j] == 0:
                        ...
                        actions.append(0)
                if step == 0:
                    obs_origin[j] = global_obs  # Store global obs
                    action_origin[j] = actions[j]
            # 记录动作分布
            ...
            print(
                f"*********************Episode {episode_gone}/{max_episode}| Step {step}/{max_step}| Reward: {total_reward:.2f}**********************")

            # 路径规划
            obsCenterNext = obsCenter
            vObsNext = vObs
            # 根据当前位置、目标位置、障碍物位置，规划避障路径，输出为下一时刻的速度矢量（包括大小和方向）
            vNext = iifds.getvNext(q, v, obsCenter, vObs, qBefore, goal, flag_car, arglist, actors_cur1,
                                   actors_cur2)
            random_q, q = q, random_q
            ...
            for j in range(iifds.numberofcar):
                if flag_car[j] == 1:
                    ...
                    if missle_index[j] != 0:
                        ...
                        if fire_car[j] >= 0:
                            if iifds.cos_cal(qBefore[int(fire_car[j])] - qBefore[j], q[int(fire_car[j])] - qBefore[j]) > np.cos(np.arctan(iifds.carR / iifds.distanceCost(qBefore[int(fire_car[j])], qBefore[j]))):
            ...
            rewards, rewards_loss = getReward1(iifds, actions, obs, global_reward, all_opp, all_nei_c2e, transformer_params['obs_dim'],
                                 rewards)
            ...
            for car_id in range(blue_agents):
                single_next_obs, _, _ = iifds.get_obs(car_id, alive_blue, all_opp, q, v, flag_car, HP_index)
                if isinstance(single_next_obs, list):
                    single_next_obs = np.array(single_next_obs, dtype=np.float32)
                if single_next_obs.ndim != 1 or single_next_obs.shape[0] != transformer_params['obs_dim']:
                    raise ValueError(
                        f"single_next_obs for agent {car_id} has shape {single_next_obs.shape}, expected ({transformer_params['obs_dim']},)")
                next_obs.append(single_next_obs)
            global_next_obs = np.array(next_obs, dtype=np.float32)
            if global_next_obs.shape != (blue_agents, transformer_params['obs_dim']):
                raise ValueError(
                    f"global_next_obs shape {global_next_obs.shape} does not match expected ({blue_agents}, {transformer_params['obs_dim']})")
            print("*******************************")

            # 检查是否结束
            blue_dones, _, global_done = iifds.get_agent_dones(flag_car)

            if not (episode_gone == 0 and step == 0):
                # 存储经验
                experiences = []
                for car_id in range(blue_agents):
                    if flag_decision[car_id] == 1 and flag_car[car_id] == 0:
                        # 决策智能体，使用实际动作和奖励
                        exp = (
                        obs_origin[car_id], action_origin[car_id], rewards[car_id], global_next_obs, blue_dones[car_id])
                        obs_origin[car_id] = global_obs
                        action_origin[car_id] = actions[car_id]
                    else:
                        # 非决策或死亡智能体，使用默认值
                        exp = (obs_origin[car_id], -1, 0.0, global_next_obs, blue_dones[car_id])  # 动作 -1 表示无效
                    experiences.append(exp)
                trainer.store_experiences(experiences)
                # 训练
                loss = trainer.train()
                if loss is not None:
        ...
        print(
            f"===========Episode {episode_gone}, Total Reward: {total_reward}, Avg Loss: {avg_loss:.4f}===========")
        print(
        ...
        # 更新训练进度
        trainer.update_training_progress(episode_gone)

        # 保存最佳模型
        if total_reward > best_reward:
            best_reward = total_reward
            trainer.agent.save_model(episode=episode_gone, reward=total_reward)
            print(f"新的最佳模型已保存，奖励: {best_reward:.2f}")

        # except Exception as e:
        #     print(f"Exception in episode {episode_gone}: {e}")

        # finally:
        if env is not None:
            env.close()  # Ensure the viewer is closed after each episode
    # 保存最终训练进度
    trainer.save_training_progress(reward_list)
