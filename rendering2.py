...
from pyglet.gl import glEnable, glDisable, glLineStipple, GL_LINE_STIPPLE

class Line(Geom):
    def __init__(self, start=(0.0, 0.0), end=(0.0, 0.0), dashed=False, linewidth=2):
        Geom.__init__(self)
        self.start = start
        self.end = end
        ...
        self.dashed = dashed
        if dashed:
            self.line_stipple = (GL_LINE_STIPPLE, (1, 0x00FF))
    ...
    def render1(self):
        ...
        if self.dashed:
            glDisable(GL_LINE_STIPPLE)  # 禁用虚线模式，恢复实线状态
