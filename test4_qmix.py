...
def get_args():
    ...
    parser.add_argument("--model-path", type=str, default="saved_models/best_tf_qix_tf_model.pth", help="模型路径")
...
def test_model(args, obsCenter):
    ...
    try:
        ...
        try:
            ...
            print(f"使用weights_only=True加载失败: {e}")
            print("尝试使用weights_only=False加载...")
            # 如果失败，则使用weights_only=False加载
            trainer.agent.load_model(args.model_path)
            print(f"成功使用weights_only=False加载模型: {args.model_path}")
    except Exception as e:
        print(f"加载模型失败: {e}")
    ...
    for episode in range(num_episodes):
        ...
        for step in range(max_steps):
            ...
            for j in range(int(iifds.numberofcar / 2)):
                ...
                situation_enemy = obs[j][int(2 * obs_dim / 4 + 1)] * 3 + obs[j][
                    int(3 * obs_dim / 4 + 1)]  # 敌方态势得分（有利态势追击，反之逃跑）
                situation_nei = obs[j][int(0 * obs_dim / 4 + 1)] * 3 + obs[j][
            ...
            # 获取动作并更新任务目标
            actions_refresh = trainer.get_actions(global_obs, flag_decision, eval_mode=True, obs_masks=global_obs_mask)  # 使用评估模式
            ...
            rewards, rewards_loss = getReward1(iifds, actions, obs, global_reward, all_opp, all_nei_c2e,
                                               transformer_params['obs_dim'],
                                               rewards)
    ...
    # 保存结果到文本文件
    file_path = os.path.join("test_results", "best_qmix_tf_model.txt")
    with open(file_path, "w", encoding='utf-8') as f:
        ...
        # 添加DQN强化学习参数
        f.write("Transformer强化学习参数:\n")
        f.write(f"obs_dim: {transformer_params['obs_dim']}\n")
        f.write(f"action_dim: {transformer_params['action_dim']}\n")
        f.write(f"d_model: {transformer_params['d_model']}\n")
        f.write(f"nhead: {transformer_params['nhead']}\n")
        f.write(f"num_layers: {transformer_params['num_layers']}\n")
        f.write(f"dropout: {transformer_params['dropout']}\n")
        f.write(f"lr: {transformer_params['lr']}\n")
        f.write(f"gamma: {transformer_params['gamma']}\n")
        f.write(f"epsilon: {transformer_params['epsilon']}\n")
        f.write(f"epsilon_min: {transformer_params['epsilon_min']}\n")
        f.write(f"epsilon_decay: {transformer_params['epsilon_decay']}\n")
        f.write(f"tau: {transformer_params['tau']}\n")
        f.write(f"buffer_size: {transformer_params['buffer_size']}\n")
        f.write(f"batch_size: {transformer_params['batch_size']}\n")
        f.write(f"sequence_length: {transformer_params['sequence_length']}\n")
        f.write(f"warmup_episodes: {transformer_params['warmup_episodes']}\n\n")
